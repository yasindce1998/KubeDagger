...
__attribute__((always_inline)) u8 is_in_container() {
    ...
    if (bpf_probe_read_kernel(&css_set, sizeof(css_set), &cur_tsk->cgroups) < 0)
        return 0;

    struct cgroup_subsys_state* css;
    if (bpf_probe_read_kernel(&css, sizeof(css), &css_set->subsys[0]) < 0)
        return 0;
    ...
    const char* name;
    if (bpf_probe_read_kernel(&name, sizeof(name), &kn->name) < 0)
        return 0;
